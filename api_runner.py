...
import requests  # For Heroku API calls
import asyncio
...
async def run_automation(request: AutomationRequest, background_tasks: BackgroundTasks):
    ...
    # Start the monitor for deployment completion
    background_tasks.add_task(monitor_for_completion)
...
# Helper function to scale Heroku dynos
def scale_heroku_dyno(dyno_type, quantity):
...
# Add a simple background task at the end of run_latest function to monitor for deployment and stop worker
async def monitor_for_completion():
    """Background task to check for deployment completion and stop worker dyno"""
    print("Starting deployment completion monitor...")
    try:
        ...
        # Loop until we find a completion marker
        while True:
            # Check for deployment file with vercel.app URL
            deploy_file = os.path.join(results_dir, "latest_deployment.txt")
            if os.path.exists(deploy_file):
                with open(deploy_file, "r") as f:
                    content = f.read()
                    if "vercel.app" in content:
                        print("Vercel deployment URL found - stopping worker dyno")
                        stop_worker_dyno()
                        break
...
def stop_worker_dyno():
    """Stop the worker dyno via Heroku API"""
    api_key = os.environ.get("HEROKU_API_KEY")
    app_name = os.environ.get("HEROKU_APP_NAME", "v0-automation")
    
    if not api_key:
        print("HEROKU_API_KEY not set - cannot stop worker dyno")
        return
    ...
    try:
        ...
        response = requests.patch(url, headers=headers, json=payload)
        print(f"Worker dyno stop response: {response.status_code}")
    except Exception as e:
        print(f"Error stopping worker dyno: {e}")
